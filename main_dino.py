...
import torchvision
import utils
import vision_transformer_mcssl as vits
from vision_transformer_dino import DINOHead
from vision_transformer_mcssl import RECHead
...
def get_args_parser():
    ...
    parser.add_argument('--drop_path_rate', type=float, default=0.05, help="stochastic depth rate")
    ...
    parser.add_argument('--teacher_temp', default=0.004, type=float, help="""Final value (after linear warmup)
    ...
    parser.add_argument('--output_dir', default="./output_dino_final_last_zeros_crop/", type=str, help='Path to save logs and checkpoints.')
    parser.add_argument('--saveckp_freq', default=20, type=int, help='Save checkpoint every x epochs.')
    parser.add_argument('--seed', default=1122, type=int, help='Random seed.')
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss,recons_loss, data_loader,
    ...
    for it, ((orig_imgs, corr_imgs, masks), _) in enumerate(metric_logger.log_every(data_loader, 100, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            # student_output1 , rec_s1= student(images[:2],rec=False)
            ...
            if plot_==True and utils.is_main_process():# and args.saveckp_freq and epoch % args.saveckp_freq == 0:
                ...
                imagesToPrint = torch.cat([orig_imgs[0][0: min(15, args.batch_size_per_gpu)].cpu(),  corr_imgs[0][0: min(15, args.batch_size_per_gpu)].cpu(),
                                       rec_s[0: min(15, args.batch_size_per_gpu)].cpu(), masks[0][0: min(15, args.batch_size_per_gpu)].cpu()], dim=0)
                torchvision.utils.save_image(imagesToPrint, print_out, nrow=min(15, args.batch_size_per_gpu), normalize=True)
...
class DINOLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        student_out = student_out.chunk(2) #n_crops
